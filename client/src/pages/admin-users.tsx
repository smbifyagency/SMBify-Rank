        ...
        <div className="py-10 px-4 sm:px-6 lg:px-8">
            <div className="max-w-7xl mx-auto">
                <h1 className="text-3xl font-bold text-white mb-2 flex items-center gap-3">
                    <Users className="h-8 w-8 text-[#7C3AED]" /> User Management
                </h1>
                <p className="text-gray-400 mb-8">Manage platform users and their accounts.</p>

                <div className="relative mb-6">
                    <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-500" />
                    <input type="text" placeholder="Search users..." className="w-full bg-white/5 border border-white/10 rounded-xl pl-11 pr-4 py-3 text-white placeholder:text-gray-500 focus:outline-none focus:border-[#7C3AED]" />
                </div>
                        ...
                        <tbody>
                            {users.map((u) => (
                                <tr key={u.email} className="border-t border-white/5 hover:bg-white/[0.02]">
                                    ...
                                    <td className="p-4 text-center"><span className={`px-2 py-0.5 rounded-full text-xs ${u.plan === "Agency" ? "bg-[#7C3AED]/8 text-[#7C3AED]" : u.plan === "Pro" ? "bg-[#7C3AED]/10 text-[#7C3AED]" : "bg-gray-500/10 text-gray-400"}`}>{u.plan}</span></td>
