      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
        <div className="rounded-2xl border border-white/10 bg-white/[0.02] backdrop-blur-sm p-6 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full border border-[#AADD00]/25 bg-[#AADD00]/10 px-3 py-1 text-xs font-semibold text-[#bef000]">
              ...
              <div className="mt-4 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
                {section.pages.map((page) => (
                  <Link
                    key={page.id}
                    href={page.href}
                    className="rounded-lg border border-white/10 px-3 py-3 text-sm text-gray-300 hover:border-[#AADD00]/40 hover:bg-indigo-500/5 hover:text-white transition-colors"
                  >
